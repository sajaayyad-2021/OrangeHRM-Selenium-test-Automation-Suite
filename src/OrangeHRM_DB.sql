...
WHERE table_schema = 'OrangeHRM_DB';
...
-- Export Latest Execution Results (CSV Format)
SELECT 
    te.execution_id as 'Execution ID',
    te.execution_name as 'Execution Name',
    te.browser as 'Browser',
    DATE_FORMAT(te.started_at, '%Y-%m-%d %H:%i:%s') as 'Started',
    ts.suite_name as 'Suite',
    tc.test_case_name as 'Test Case',
    tr.comparison_result as 'Result',
    ROUND(tr.duration_ms / 1000.0, 2) as 'Duration (s)',
    tr.expected_value as 'Expected',
    tr.actual_value as 'Actual'
FROM test_results tr
JOIN test_cases tc ON tr.test_case_id = tc.test_case_id
